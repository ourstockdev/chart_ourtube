...
class ChartPainter extends BaseChartPainter {
  ...
  /* 1일 단위로 선택했는지 여부. 1일 단위인 경우, 연말->연초로 넘어갈 때 년도를 따로 표시한다. */
  final bool is1Day;
  ...
  void drawDate(Canvas canvas, Size size) {
    ...
    for (var i = 0; i <= mGridColumns; ++i) {
      ...
      if (translateX >= startX && translateX <= stopX) {
        ...
        y = size.height - (mBottomPadding - tp.height) - tp.height + 16; //size.height - (mBottomPadding - tp.height) / 2 - tp.height;
...
//    }
  ...
  void drawMaxAndMin(Canvas canvas) {
    ...
    TextPainter tp = getTextPainter(
        '최저 ' + getFormattingNumber(mMainLowMinValue), chartColors.minColor);
    if (x < tp.width / 2) { //x < mWidth/2
      //画右边
      TextPainter tp = getTextPainter(
          '── 최저 ' + getFormattingNumber(mMainLowMinValue), chartColors.minColor);
      tp.paint(canvas, Offset(x, y - tp.height / 2));
    } else if(x > mWidth-tp.width / 2) {
      TextPainter tp = getTextPainter(
          '최저 ' + getFormattingNumber(mMainLowMinValue) + ' ──', chartColors.minColor);
    ...
    TextPainter tp2 = getTextPainter(
        '최고 ' + getFormattingNumber(mMainHighMaxValue), chartColors.maxColor);
    if (x < tp2.width / 2) {
      //画右边
      TextPainter tp = getTextPainter(
          '── 최고 ' + getFormattingNumber(mMainHighMaxValue), chartColors.maxColor);
      tp.paint(canvas, Offset(x, y - tp.height / 2));
    } else if(x > mWidth-tp2.width / 2) {
      TextPainter tp = getTextPainter(
          '최고 ' + getFormattingNumber(mMainHighMaxValue) + ' ──', chartColors.maxColor);
      tp.paint(canvas, Offset(x - tp.width, y - tp.height / 2));
    } else {
      tp2.paint(canvas, Offset(x - tp2.width / 2, y - tp2.height)); // - tp2.height / 2
    }
  }
  ...
  void drawNowPrice(Canvas canvas) {
    ...
    nowPricePaint
      ..color = this.chartColors.nowPriceColor;
    //先画横线
    double startX = 0;
    ...
    //再画背景和文本
    TextPainter tp = getTextPainter(getFormattingNumber(value), this.chartColors.nowPriceTextColor);
    double left = 0;
    double top = y - tp.height / 2;
    canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTRB(left, top, left + tp.width, top + tp.height), Radius.circular(2)), nowPricePaint);
  ...
  void drawCrossLine(Canvas canvas, Size size) {
    ...
    Paint paintX = Paint()
      ...
      ..isAntiAlias = true;
    // 현재 가격에서 종가에 해당하는 지점?을 실선이 아닌 점선으로 표시함.
